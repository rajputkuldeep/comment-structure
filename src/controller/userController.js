...
module.exports = {
  ...
  sendFriendReq: (req, res) => {
    ...
    User.findOne({_id: Mongoose.Types.ObjectId(reqParam.reqId)}).then((data) => {
      if (!data) {
  ...
  acceptRejectReq: (req, res) => {
    ...
    User.findOne({_id: Mongoose.Types.ObjectId(reqParams.authId)}).then((data) => {
      const check = data.friend_request.filter((x) => x.toString() === reqParams.fri)
      if (check.length === 1) {
        ...
        if (reqParams.type === 1) {
          ...
          User.findOne({_id: Mongoose.Types.ObjectId(reqParams.fri)}).then((friData) => {
  ...
  createCustomer: (req, res) => {
    ...
    User.findOne({email:'kuldeep@mailinator.com'}).then((user)=>{
      ...
      stripe.customers.create({ email: user.email, name: user.name }, async(error, customer) => {
        if (error) {
            console.error(error);
  ...
  addCard: (req, res) =>{
    ...
    stripe.tokens.create({card:tokenObj}).then(async(token)=>{
      ...
      stripe.customers.createSource(reqParams.customer_id,{
        source: `${token.id}`
      }).then((card)=>{
        res.send(card.id)
      })
